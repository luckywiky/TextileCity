...
namespace TextileCity.Operation
{
    ...
    {
        private readonly MaterialData dal = new MaterialData();
		...
		/// 得到一个对象实体
		/// </summary>
		public Material GetModel(int id)
		{
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Material GetModelByCache(int id)
		{
			
			string CacheKey = "materialModel-" + id;
			...
			if (objModel == null)
			{
				try
				{
					...
					if (objModel != null)
					{						
						Common.DataCache.SetCache(CacheKey, objModel, DateTime.Now.AddMinutes(Common.DataCache.DefaultTime), TimeSpan.Zero);
					}
				}
				catch{}
			}
			return (Material)objModel;
		}
